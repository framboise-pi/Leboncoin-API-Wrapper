...
class CloudScraper(Session):
    ...
    def debugRequest(req):
        try:
            ...
            print("Debug Error: {getattr(e, 'message', e)}")
    ...
    def decodeBrotli(self, resp):
        if requests.packages.urllib3.__version__ < '1.25.1' and resp.headers.get('Content-Encoding') == 'br':
            if self.allow_brotli and resp._content:
                ...
                logging.warning(
                    'You\'re running urllib3 {requests.packages.urllib3.__version__}, Brotli content detected, '
        ...
        if self.is_Challenge_Request(response):
            ...
            if self._solveDepthCnt >= self.solveDepth:
                ...
                self.simpleException(
                    CloudflareLoopProtection,
                    "!!Loop Protection!! We have tried to solve {_} time(s) in a row."
                )
    ...
    def IUAM_Challenge_Response(self, body, url, interpreter):
        ...
        try:
            ...
            self.simpleException(
                CloudflareIUAMError,
                "Unable to parse Cloudflare anti-bots page: {getattr(e, 'message', e)}"
            )

        return {
            'url': "{hostParsed.scheme}://{hostParsed.netloc}{self.unescape(formPayload['challengeUUID'])}",
            'data': payload
        }
    ...
    def Challenge_Response(self, resp, **kwargs):
        ...
        if submit_url:
            ...
            cloudflare_kwargs['headers'] = updateAttr(
                ...
                'headers',
                {
                    'Origin': '{urlParsed.scheme}://{urlParsed.netloc}',
                    'Referer': resp.url
                }
            )
    ...
    def get_tokens(cls, url, **kwargs):
        ...
        try:
            ...
            logging.error('"{url}" returned an error. Could not collect tokens.')
            raise
...
if ssl.OPENSSL_VERSION_INFO < (1, 1, 1):
    print(
        "DEPRECATION: The OpenSSL being used by this python install ({ssl.OPENSSL_VERSION}) does not meet the minimum supported "
